...
test('room system with multiple users', async () => {
  ...
  try {
    ...
    await expect(alicePage.locator(`h1:has-text("Room: ${roomCode}")`)).toBeVisible();
    await expect(alicePage.locator('.users-list li:has-text("Alice")')).toBeVisible();
    await expect(alicePage.locator('button:has-text("Share Link")')).toBeVisible();
    await expect(alicePage.locator('button:has-text("Join Room")')).not.toBeVisible();
    ...
    await expect(bobPage.locator('.users-list li:has-text("Bob")')).toBeVisible();
    await expect(bobPage.locator('.users-list li:has-text("Alice")')).toBeVisible();
    await expect(bobPage.locator('button:has-text("Join Room")')).not.toBeVisible();
    
    // Verify Alice can see Bob
    await expect(alicePage.locator('.users-list li:has-text("Bob")')).toBeVisible();
    ...
    await expect(charliePage.locator('.users-list li:has-text("Charlie")')).toBeVisible();
    await expect(charliePage.locator('.users-list li:has-text("Alice")')).toBeVisible();
    await expect(charliePage.locator('.users-list li:has-text("Bob")')).toBeVisible();
    await expect(charliePage.locator('.users-list li')).toHaveCount(3);
    
    // Verify Alice and Bob can see Charlie
    await expect(alicePage.locator('.users-list li:has-text("Charlie")')).toBeVisible();
    await expect(alicePage.locator('.users-list li')).toHaveCount(3);
    
    await expect(bobPage.locator('.users-list li:has-text("Charlie")')).toBeVisible();
